  ...
  <q-page class="bb-goals q-pa-lg">
    <div class="bb-page-header bb-budgets-header">
      <div>
        ...
        <div class="bb-page-sub">Earmark the money in your savings toward the things you're saving up to buy</div>
      </div>
      <q-btn
      ...
      <div class="row q-col-gutter-md q-mb-lg">
        <div class="col-6 col-md-4">
          <div class="bb-stat bb-stat--gradient">
            <div class="bb-stat-lbl"><span class="bb-dot" style="background:rgba(255,255,255,0.6)"></span>IN SAVINGS</div>
            <div class="bb-stat-val">{{ fmt(data.inSavings) }}</div>
            <div class="bb-stat-row"><span class="bb-stat-cmp">{{ fmt(data.savedThisMonth) }} moved in this month</span></div>
          </div>
        </div>
        <div class="col-6 col-md-4">
          <div class="bb-stat">
        ...
        <div class="col-6 col-md-4">
          <div class="bb-stat">
            ...
            <div class="bb-stat-val">{{ fmt(data.availableForGoals) }}</div>
            <div class="bb-stat-row">
              <span class="bb-stat-cmp">savings not yet earmarked to a goal</span>
            </div>
          </div>
        </div>
      </div>

      <!-- Reminder: move money into savings to hit this month's 20% -->
      <q-banner v-if="data.moveReminder.needed > 0" class="bb-move-banner q-mb-lg" rounded>
        <template #avatar><q-icon name="account_balance" color="amber" /></template>
        You've saved <strong>{{ fmt(data.moveReminder.savedThisMonth) }}</strong> of your
        <strong>{{ fmt(data.moveReminder.targetThisMonth) }}</strong> savings target this month —
        move <strong>{{ fmt(data.moveReminder.needed) }}</strong> more into your savings account to stay on track.
      </q-banner>
      ...
      <div class="bb-bg-panel bb-reserve q-mb-lg">
        <div class="bb-bg-panel-hdr">
          ...
          <span class="bb-bg-panel-sub">
            Tracked automatically from the money you actually move into your Savings — aim to set aside
            {{ (data.reservePct * 100).toFixed(0) }}% of what you earn each month.
          </span>
          <q-space />
          <q-chip
            v-if="data.reserve.remainingThisMonth > 0"
            dense square
            style="background:rgba(245,158,11,0.15);color:#F59E0B;font-weight:600"
          >
            Move {{ fmt(data.reserve.remainingThisMonth) }} more
          </q-chip>
          <q-badge v-else color="green" class="bb-goal-done">On track ✓</q-badge>
        </div>
        <div class="bb-bg-row">
        ...
        <div v-else class="bb-bg-list">
          <div v-for="g in data.goals" :key="g.id" class="bb-bg-row">
            ...
            <q-btn
              ...
              :disable="data.availableForGoals <= 0 || g.remaining <= 0"
              @click="openAddMoney(g)"
            >
              <q-tooltip>{{ data.availableForGoals <= 0 ? 'No unearmarked savings available' : 'Earmark savings for this goal' }}</q-tooltip>
            </q-btn>
      ...
      <q-card class="bb-planner-card" style="width:380px">
        ...
        <div class="bb-goal-form">
          <q-input
            ...
            :hint="`${fmt(data?.availableForGoals ?? 0)} available · ${fmt(addMoneyGoal?.remaining ?? 0)} left on this goal`"
          />
    ...
    <q-dialog v-model="plannerOpen">
      <q-card class="bb-planner-card">
        ...
        <div v-else-if="!plan.length" class="bb-planner-empty">
          Nothing to allocate — add a goal, or move some money into savings first.
        </div>
...
const addMoneyMax = computed(() =>
  Math.min(data.value?.availableForGoals ?? 0, addMoneyGoal.value?.remaining ?? 0),
);
...
// Pop up once per session nudging you to move money into savings when this
// month's real deposits fall short of the 20% "pay yourself first" target.
function maybeRemindToMove() {
  const r = data.value?.moveReminder;
  if (!r || r.needed <= 0) return;
  if (sessionStorage.getItem('bb_savings_move_reminded') === '1') return;
  sessionStorage.setItem('bb_savings_move_reminded', '1');
  $q.dialog({
    title: 'Move money into savings',
    message: `You've saved ${fmt(r.savedThisMonth)} of your ${fmt(r.targetThisMonth)} savings target this month. Transfer ${fmt(r.needed)} more into your savings account to hit your 20%.`,
...
async function generate() {
  ...
  try {
    ...
    if (res.available <= 0) {
      $q.notify({ type: 'info', message: 'No unearmarked savings to allocate yet — move money into savings first.' });
    }
  } catch (e) {
