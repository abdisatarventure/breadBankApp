...
export interface Subscription {
  ...
  periodDays: number;           // days between charges, for projecting a calendar
...
// "AMAZON PRIME*X50319W53 AMZN.CO". Strip those so identical recurring charges
// collapse to one merchant instead of scattering into many one-off groups.
function normalizeMerchant(raw: string): string {
  ...
  const kept = tokens.filter((tok) => {
    ...
    if (isUpper && hasLetter && digits >= 2 && tok.length >= 6) return false; // ref code
    return true;
  });
...
// Detect active recurring charges from the last 13 months of debits. Shared by
// the Subscriptions page and the Bill calendar so both agree on what's recurring.
export async function detectSubscriptions(userId: number): Promise<Subscription[]> {
